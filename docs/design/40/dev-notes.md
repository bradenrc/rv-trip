...
`mc/dev/issue-40-v0` was cut before `0a9284c feat(19): Adopt Tailwind Slate +
Sky palette (known system) + light/dark toggle (#49)` landed on `main`. That
commit is a pure restyle, so every hunk below is integration only — main moved
the palette, and the feature's new call sites had to follow. **No feature
behaviour was changed, added, or dropped.**

### What conflicted (2 files, 2 hunks)

`git merge --no-ff mc/dev/issue-40-v0` off current `origin/main` conflicted in
exactly the two places where #19 restyled a line this branch also rewrote.

| # | file | main's side | this branch's side | resolution |
|---|---|---|---|---|
| 1 | `apps/web/src/components/trip/TripPlanner.tsx:747` (masthead "Add stop") | recoloured the CTA `bg-rv-ember`/`text-rv-navy` → `bg-rv-accent-deep`/`text-rv-accent-ink` | added the `onClick` (append to `legOrder(trip).at(-1)`), `disabled={trip.legs.length === 0}` and the `disabled:` classes | **both**: feature's `onClick`/`disabled` kept verbatim, className carries main's accent pair plus the feature's `disabled:cursor-default disabled:opacity-45` |
| 2 | `apps/web/src/components/trip/StopDetailSheet.tsx:263` (reservation add form) | recoloured the single "Save reservation" button | replaced that lone button with the check-in/check-out row, Confirmation #, and a Save/Cancel pair (i2's edit mode) | **both**: feature's whole block kept, main's rename applied to the Save button inside it |

Neither hunk needed feature judgment: the two sides touched disjoint
properties of the same JSX element (main the `className`, the branch the
handlers/markup), so preserving both intents is mechanical.

### CI drift the merge did not flag

`packages/core/src/theme/nightfall-tokens.test.ts:317` — *"retires the
`rv-ember-*` family from every call site"* — scans **all** of
`packages/ui/src` + `apps/web/src`, so it reds on lines that merged cleanly.
#19 deleted `--color-rv-ember{,-bright,-deep,-soft}` from both stylesheets
(they are the only tokens the palette commit removed; `git diff` of the
`@theme` blocks confirms nothing else was renamed), and this branch had
authored 7 new call sites against the retired names.

Renamed with #19's own two substitutions, verified against its diff of the
pre-existing sites:

- CTA fill pair `bg-rv-ember` + `text-rv-navy` → `bg-rv-accent-deep` + `text-rv-accent-ink`
- mono kicker `text-rv-ember` → `text-rv-accent`

| site | shape |
|---|---|
| `apps/web/src/app/trips/new/page.tsx:44` | kicker |
| `apps/web/src/app/trips/new/page.tsx:107` | CTA (Create trip) |
| `apps/web/src/components/trip/StopDetailSheet.tsx:378` | CTA (idea save) |
| `apps/web/src/components/trip/StopDetailSheet.tsx:436` | CTA (promote) |
| `apps/web/src/components/trip/TripPlanner.tsx:1166` | CTA (add leg) |
| `apps/web/src/components/trip/TripPlanner.tsx:1231` | CTA (add stop, leg header) |
| `apps/web/src/components/trip/TripPlanner.tsx:1361` | CTA (add idea) |

The rename is paired on purpose: the same test file asserts both directions of
`bg-rv-accent-deep` ⇄ `text-rv-accent-ink` (`sweep 1 (#19)`) and bans a bare
`text-rv-navy` off a green background, so swapping only the fill would have
traded one red for another.

Also re-checked, and clean, after the merge — the other #19 sweeps that scan
every source file (`text-rv-navy` off green, accent pair both directions,
inline `color: var(--color-rv-navy)`, `bg-rv-green-cta`,
`text-/border-rv-surface`, `text-rv-navy-soft`, `text-white` off
`bg-rv-navy`, `rv-ink-subtle`). This branch's two new components
(`components/trip/row-menu.tsx`, `components/ui/inline-text.tsx`) only ever
reference surviving tokens, so neither needed an edit.

### Not touched, and why

- **Design tokens** — `packages/ui/styles/entry.css` and
  `apps/web/src/app/globals.css` are the pair that must be mirrored, and
  neither side of the merge changed either file (this branch ships no token).
  The drift guard passes, so the DS and the app are still byte-consistent.
- **Migrations** — this branch's `packages/db/src/schema.ts` change
  (`status_auto`) merged cleanly and there is still no `packages/db/drizzle/`;
  the repo's `db:push` path is unchanged, so nothing to regenerate.
- **Lockfile** — no `package.json` on either side changed, and
  `pnpm install --frozen-lockfile` succeeded against the merged tree, so the
  ship's setup step will too.
- **The three `.dark` chrome islands** — #19's `§5` assertions
  (`Nav.tsx`, `StopDetailSheet.tsx:163`, `MapOverview.tsx`) all auto-merged and
  survive; verified by grep and by the green test.

### Checks run

| check | command | result |
|---|---|---|
| lockfile | `pnpm install --frozen-lockfile` | `Lockfile is up to date, resolution step is skipped` · `Done in 6.4s` |
| typecheck | `pnpm turbo run typecheck` | `Tasks:    6 successful, 6 total` |
| lint | `pnpm turbo run lint` | `Tasks:    2 successful, 2 total` (`@rv-trip/web:lint` clean) |
| test | `pnpm turbo run test` | `Test Files  20 passed (20)` · `Tests  334 passed (334)` — incl. `src/theme/nightfall-tokens.test.ts (30 tests)` |
| build | `pnpm turbo run build` | `Tasks:    2 successful, 2 total`; all four new routes present (`/api/legs`, `/api/stops`, `/api/ideas`, `/api/trips/[id]/legs/reorder`) |

**SKIPPED (no env):** no browser/runtime check and no database check — same
gap as the dev round. The two render-required flags the vet raised
(gap-targeted drop in `Timeline.tsx`; Radix portals inside the hand-rolled
sheet) are untouched by this fixup and remain the walk gate's to confirm.
