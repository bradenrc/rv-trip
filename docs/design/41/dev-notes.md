...
# Issue 41 — dev notes · **Fixup round** (re-land on current `origin/main`)

Ship blocked `[merge-conflict]` on `mc/dev/issue-41-v0`. Integration only — no feature
judgment was exercised, no behaviour was changed, nothing outside the merge's blast radius
was touched.

## What main did underneath the branch

Exactly **one** commit of drift since the merge base (`7f51341`):

- `0a9284c` — `feat(19): Adopt Tailwind Slate + Sky palette (known system) + light/dark` (#49).

That commit retired the whole `rv-ember-*` token family — renamed to `rv-accent-*` and
revalued to Sky — and re-pointed the accent CTA pair. Confirmed mechanically by diffing the
token names across it (`packages/ui/styles/entry.css`):

- removed: `--color-rv-ember`, `-bright`, `-deep`, `-soft`
- added: `--color-rv-accent`, `-bright`, `-deep`, `-soft`, **`-ink`** (new)

`git merge --no-ff mc/dev/issue-41-v0` reproduced the ship's failure exactly: one conflicted
file, `apps/web/src/app/places/page.tsx`.

## Conflict 1 — `apps/web/src/app/places/page.tsx` (the only `UU`)

The two sides touched the same JSX region for unrelated reasons:

- **main** restyled the "Save a place" CTA in place: `bg-rv-ember … text-rv-navy` →
  `bg-rv-accent-deep … text-rv-accent-ink`.
- **the feature** deleted that CTA *from the page* and moved it into the new client island,
  `PlacesWorkspace` (`apps/web/src/components/places/PlacesWorkspace.tsx:190-200`), together
  with the wrapper `<div>` and `<PlacesLibrary>`, because the button now needs an `onClick`
  that opens the save sheet.

**Resolved to the feature side**, and this is integration rather than judgment because the
island absorbs main's hunk verbatim rather than discarding it: `PlacesWorkspace.tsx:191-199`
renders the same wrapper div, the same `<Plus>` glyph and the same CTA classes. Nothing main
wrote is lost — it relocated one file over. main's *other* edit to this file, the kicker
`text-rv-ember` → `text-rv-accent` (`page.tsx:35`), sat outside the conflict region and git
auto-merged it correctly.

The dropped `import { Plus } from "lucide-react"` is correct: the only `<Plus>` on the page
moved into the island, which imports it itself.

## Conflict 2 (silent) — the palette rename never reached the branch's new files

This is the `[ci-red]` half that a clean `git merge` would have shipped: the feature authored
**four new/edited files against the retired token family**, and git had no conflict to raise
because main never touched those paths. main guards exactly this with a source-text sweep in
`packages/core/src/theme/nightfall-tokens.test.ts` — `sourceFiles()` (`:337-349`) walks *all*
of `packages/ui/src` and `apps/web/src`, so the branch's brand-new files are inside the sweep.
Three of its assertions were red on the raw merge:

- `:317` "retires the `rv-ember-*` family from every call site" — 4 hits.
- `:280-286` sweep 1 (#3): `text-rv-navy` only survives on `bg-rv-green*` — 3 hits.
- `:289-297` sweep 1 (#19): `bg-rv-accent-deep` and `text-rv-accent-ink` imply each other.

Applied main's own mapping, taken verbatim from the #49 diff rather than invented:

| file:line | before | after |
| --- | --- | --- |
| `apps/web/src/components/places/PlacesWorkspace.tsx:195` | `bg-rv-ember … text-rv-navy` | `bg-rv-accent-deep … text-rv-accent-ink` |
| `apps/web/src/components/places/SheetShell.tsx:84` | `bg-rv-ember … text-rv-navy` | `bg-rv-accent-deep … text-rv-accent-ink` |
| `apps/web/src/components/places/Suggestions.tsx:106` | `bg-rv-ember … text-rv-navy` | `bg-rv-accent-deep … text-rv-accent-ink` |
| `apps/web/src/components/places/PlacePicker.tsx:289` | `text-rv-ember` | `text-rv-accent` |

The CTA row is the pair main itself moved the seven existing CTAs onto in #49 (the same
`bg-rv-ember … text-rv-navy` → `bg-rv-accent-deep … text-rv-accent-ink` edit it made to this
very `places/page.tsx` button); the picker's line is a mono meta glyph, which is the plain
`text-rv-ember` → `text-rv-accent` rename main applied to every kicker. "main moved; the
feature had to follow" in both cases — no colour was chosen here.

## What did NOT need doing

- **Token mirroring.** The `rv-*` blocks are duplicated in `packages/ui/styles/entry.css` and
  `apps/web/src/app/globals.css`, but the feature branch touches **neither** file, so both
  arrived from main already identical — verified by extracting the token-name sets from each
  (38 names, equal). No mirroring edit was made, and none was needed.
- **Lockfile.** The branch's `package.json` edits add **no dependencies** — only two scripts
  (`backfill:places` in root and `packages/db`) and one `exports` subpath
  (`./providers/google-places` in `packages/core`). The dependency graph is unchanged, so no
  `pnpm install --no-frozen-lockfile` re-resolution was performed; `--frozen-lockfile` was run
  instead and passed, which is the same command the ship's setup step uses.
- **Drizzle migration.** The branch generates none (the design's no-migration finding stands:
  `saved_places.lat/lng` are already nullable), and `packages/db/drizzle/` still does not exist.
- **`packages/ui` API.** #49's edit to `packages/ui/src/Places.tsx` is className-only — no
  export, prop or signature moved — so the branch's `PlaceCard` use needed no follow.
- `apps/web/src/components/map/MapOverview.tsx` conflicted at the ship only as an
  `Auto-merging` line, not a `CONFLICT`; the merge correctly kept main's palette edits
  (`bg-rv-ember`→`bg-rv-accent`, `border-rv-ember-deep`→`border-rv-accent-deep`, the new
  `.dark` legend bar and the `TeardropKey` className rewrite) alongside the feature's Locate
  button. Left as git resolved it; verified by diffing the merged file against the branch's.

## Checks run

- `git merge --no-ff mc/dev/issue-41-v0` → `CONFLICT (content): Merge conflict in
  apps/web/src/app/places/page.tsx` (one file; reproduces the ship's failure verbatim).
- `pnpm install --frozen-lockfile` → `Lockfile is up to date, resolution step is skipped` …
  `Done in 6.2s`. This is the ship's own setup command, so the lockfile is proven, not assumed.
- `pnpm turbo run lint typecheck test` → `Tasks: 8 successful, 8 total`, with
  `@rv-trip/core:test: Tests 360 passed (360)` across 21 files — including
  `src/theme/nightfall-tokens.test.ts (30 tests)`, the sweep that was red before the rename,
  and all seven of the branch's own new suites (`place-picker` 38, `places-locate` 24,
  `places-search` 22, `places` 23, `place-form` 23, `google-places` 15, `types` 14).
- Token-reference audit over the 18 `.ts`/`.tsx` files the branch adds or edits under
  `apps/web/src` + `packages/ui/src`: every `bg-/text-/border-/rounded-/shadow-rv-*` utility
  and every inline `var(--color-rv-*)` resolves against the merged token set →
  `all rv-* utilities in feature files resolve against merged tokens`. `grep -rn rv-ember
  apps/web/src packages/ui/src` → no hits.
