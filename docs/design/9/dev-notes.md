...
## Fixup round — re-landing on main after issue 11

The ship failed with `[merge-conflict]`: `main` moved to `2cc16bb` (issue 11, the Mapbox
map surfaces) while this branch was in the walk. Re-landed as
`git merge --no-ff mc/dev/issue-9-v0` onto a fresh branch off `origin/main`. Integration
only — no behavior on either side was changed.

### The two conflicts, and how each was resolved

Both were **additive collisions on the same line** — two siblings appending to the same
barrel/import list — so both resolved as a **union of both sides**. Neither required
feature judgment: every symbol on both sides is live in the merged body.

| file | HEAD (`main`/issue 11) | branch (issue 9) | resolution |
| --- | --- | --- | --- |
| `packages/core/src/domain/index.ts:3-5` | `export * from "./bounds"` | `export * from "./rig"` + `"./route-order"` | all three re-exported; the three modules are disjoint and all exist on disk |
| `apps/web/src/components/trip/TripPlanner.tsx:6` | `import { isScheduled }` | `import { orderedPairs, routeCacheKey }` | one merged import — `isScheduled` is used by issue 11's `scheduledOrdinal` memo (`:96`), `orderedPairs`/`routeCacheKey` by issue 9's `upgradeRoutes` (`:178-179`) |

Git auto-merged the rest of `TripPlanner.tsx` cleanly: issue 11's `scheduledOrdinal` memo
and issue 9's `routes`/`rigHash` props and `upgradeRoutes` are disjoint hunks in the same
component. `packages/db/src/queries.ts` and `packages/ui/src/index.ts` auto-merged (both
sides appended distinct exports).

### The one integration fix CI needed

`pnpm turbo run lint typecheck test` came back red on exactly one line:

```
apps/web/src/components/map/pins.ts(5,21): error TS2305:
  Module '"@/lib/trip-ui"' has no exported member 'estimateDrive'.
```

Textbook drift: issue 9 **deleted** `estimateDrive` from `apps/web/src/lib/trip-ui.tsx`
(collapsing the duplicate haversine into core's single `estimateRoute`, per the vet's §7
#9a finding), while issue 11 — branched before that landed — **added a new consumer** of
it in `pins.ts` for the map's estimated-drive arcs. Neither side saw the other; the merge
is textually clean and only the type checker catches it.

Repointed the consumer at the surviving implementation, `pins.ts:3-5` and `:191-198`:

- `import { dateRange, estimateDrive } from "@/lib/trip-ui"` → `import { dateRange }`,
  with `driveMiles, estimateRoute` added to the existing `@rv-trip/core` import.
- `const d = estimateDrive(a, b); if (!d) continue;` → `const miles = driveMiles(estimateRoute(a, b));`

The `if (!d) continue` guard drops out because it was already dead: `estimateRoute` takes
`LatLng` and never returns null, and the call site is guarded two lines up by
`hasCoords(a) || hasCoords(b)`. That matches the vet's resolution of the state-3
contradiction — **missing coordinates mean no connector and no arc, never a neutral
`estimate` row**.

**The rendered miles are unchanged.** Old: `Math.round(km * 0.621371)`. New:
`Math.round(Math.round(km * 1000) / 1609.344)`. Same haversine, same value. The 75 vs
80 km/h difference between the two implementations only ever moved *minutes*, and
`pins.ts` reads miles alone. Issue 11's own comment — that the map and the Route rail can
never print different numbers — is now literally true rather than aspirational, since
both sides call the same core function; the comment at `pins.ts:11-13` was updated to
name `estimateRoute()` instead of the deleted symbol.

### Derived files — checked, none needed regenerating

- **`pnpm-lock.yaml`** — no conflict, and none possible: issue 9 touched no dependency.
  Its only `package.json` edit is an export subpath (`packages/core/package.json:9`,
  `"./providers/here"`). `pnpm install --frozen-lockfile` — the exact command the ship's
  setup step runs — reported `Lockfile is up to date, resolution step is skipped`, so the
  lockfile was deliberately left as `main`'s.
- **`rv-*` design tokens** — `packages/ui/styles/entry.css` and
  `apps/web/src/app/globals.css` were touched by *neither* branch
  (`git diff --name-only 67f89aa --` on both paths is empty). The two copies are still in
  sync; `--color-rv-travel{,-ink,-soft}` is present in both at `entry.css:55-57` /
  `globals.css:162-164`. Nothing to mirror.
- **Drizzle migration** — none to regenerate. `packages/db/src/schema.ts` grew the `rigs`
  table on this branch, but `packages/db/drizzle/` still does not exist and the repo is
  still on `pnpm db:push`, exactly as before the merge.

### Checks run — fixup round

| check | command | result |
| --- | --- | --- |
| install | `pnpm install --frozen-lockfile` | `Lockfile is up to date, resolution step is skipped` … `Done in 3.9s` |
| gate | `pnpm turbo run lint typecheck test` | `Tasks: 7 successful, 7 total` |
| unit tests | (same run) `@rv-trip/core:test` | `Test Files 10 passed (10)` · `Tests 142 passed (142)` — issue 9's 127 plus issue 11's 15 `bounds` tests |
| conflict sweep | `grep -rn '<<<<<<<\|>>>>>>>'` over `ts,tsx,css,json,yaml` | `NONE` |

**SKIPPED (unchanged from round 3, not re-verified here):** `pnpm build`, `pnpm db:push`,
`pnpm db:seed`, and every live-vendor check (HERE OAuth/routing/notices, the Google Maps
handoff, the Playwright browser passes). This round changed two import lines and one
call site; no vendor path, no schema, and no rendered string was touched. Round 3's
