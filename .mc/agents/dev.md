...
2. **TDD:** write a failing test for the change → implement the minimal code → make it pass.
   **Discover the runners from the workspace** (any package.json with a `test` script) —
   today that is `vitest` in BOTH `packages/core` and `apps/web` (the route-handler
   integration suite against real Postgres, per #30). Pure logic still belongs down in
   `packages/core` (that is what the package is for); route-handler behavior is tested
   where it lives, in `apps/web`. If a change genuinely can't be covered either way, say so
...
## Machine conduct — you share this machine

Other issues' walk servers, dev servers, and simulators run beside yours. **Never kill
processes machine-wide** — no `pkill -f "next dev"`, no `killall node`, nothing pattern-wide
(one worker's pkill once took down every other issue's walk server, #48). Kill only what you
own, by PID or by your own port/slug: your worktree's dev server via its recorded pid file or
`lsof -ti :<your-port>`. If a port you expected is held by a process that isn't yours, pick
another port or say so in dev-notes — do not clear it.
